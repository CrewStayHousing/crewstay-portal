          ...
          {selProject && (
            <>
              ...
              <div style={S.tableCard}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '10px 20px', background: '#F7F8FA', borderBottom: '1px solid #E8ECF2' }}>
                  {['Property', 'Beds', 'EPC/mo', 'Landlord/mo', 'Spread/mo'].map(h => (
                    <div key={h} style={S.th}>{h}</div>
                  ))}
                </div>
                {projectProperties.map(p => (
                  <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '14px 20px', borderBottom: '1px solid #F7F8FA', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontWeight: 700, color: '#1B2A4A', fontSize: 14 }}>{p.address}</div>
                      <div style={{ fontSize: 12, color: '#6B7A99' }}>{p.city}, GA</div>
                    </div>
                    <div style={{ fontWeight: 700, color: '#1B2A4A' }}>{p.bedrooms}</div>
                    <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#1B2A4A' }}>${Number(p.monthly_client_rate).toLocaleString()}</div>
                    <div style={{ color: '#6B7A99', fontWeight: 600 }}>${Number(p.monthly_landlord_rate).toLocaleString()}</div>
                    <div style={{ fontWeight: 700, color: '#27AE60' }}>${(Number(p.monthly_client_rate) - Number(p.monthly_landlord_rate)).toLocaleString()}</div>
                  </div>
                ))}
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '14px 20px', background: '#F7F8FA', borderTop: '2px solid #E8ECF2' }}>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#1B2A4A' }}>TOTAL</div>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#1B2A4A' }}>{contractedBeds}</div>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#1B2A4A' }}>${projectMonthlyBilling.toLocaleString()}</div>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#6B7A99' }}>${projectProperties.reduce((s, p) => s + Number(p.monthly_landlord_rate), 0).toLocaleString()}</div>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: '#27AE60' }}>${projectProperties.reduce((s, p) => s + Number(p.monthly_client_rate) - Number(p.monthly_landlord_rate), 0).toLocaleString()}</div>
                </div>
              </div>
            </>
          )}
